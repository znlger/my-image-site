...
export async function onRequestPost({ request, env }) {
  ...
  if (!env.IMAGES) return json({ error: "R2 binding IMAGES is missing. Add binding: IMAGES → female-star-images, then redeploy." }, 500);
  if (!env.IMAGE_BASE_URL) return json({ error: "IMAGE_BASE_URL is missing. Add it in Variables and Secrets, then redeploy." }, 500);
