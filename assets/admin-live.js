...
function renderImages() {
  const g = galleries[currentIndex];
  document.querySelector("#detailPreview").innerHTML = (g.images || []).map((src, i) => `<div class="image-card"><img src="${src}"><button onclick="removeImage(${i})">×</button></div>`).join("");
}
...
function addGallery(save = true) {
  ...
  galleries.push({ id:`gallery-${String(next).padStart(3,"0")}`, title:"新图集标题", category:"活动", date:"今天", cover:"", tags:["生图"], images:[] });
...
async function uploadCover(e) {
  ...
  try {
    toast("封面上传中..."); logDebug(`开始上传封面：${file.name}\n大小：${file.size} bytes\n类型：${file.type}`);
    const result = await uploadOne(file, "cover", 1);
    galleries[currentIndex].cover = result.url; document.querySelector("#coverPreview").src = result.url; renderList();
    await saveGalleries(); toast("封面已上传并保存");
  } catch (err) { logDebug("封面上传失败：\n" + err.message); toast("封面上传失败：" + err.message); }
...
async function uploadDetails(e) {
  ...
  try {
    ...
    for (let i=0;i<files.length;i++) {
      logDebug(`开始上传详情图 ${i+1}/${files.length}：${files[i].name}`);
